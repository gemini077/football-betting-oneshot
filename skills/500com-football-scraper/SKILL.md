...
中国竞彩主源 + 500比赛ID发现 + 6个深层分析页，输出带时间戳的完整JSON批次。

```bash
python scripts/fetch_football_data.py --date 2026-07-15 --match 法国 --deep
```

### Quick Mode

只抓中国竞彩主源和500比赛列表，不抓6个深层页。

```bash
python scripts/fetch_football_data.py --date 2026-07-15 --match 法国
```
...
拿到 match_list 后，对每场比赛的 shuju_id 逐场检查共享缓存:
  ...
  对 match_list 中的每场比赛:
    ...
    cache_path = data/source_cache/shared-football/parsed/{date}_{shuju_id}.json
...
1. 对该场的每个页面类型（ouzhi/yazhi/rangqiu/daxiao/shuju/touzhu）:
   → 检查 data/source_cache/shared-football/raw/{date}/{page}_{shuju_id}.html 是否存在
   → 存在则跳过该页的 WebFetch，直接解析
   → 不存在才 WebFetch → 保存到 raw/ → 解析

2. 该场 6 页全部就绪后:
   → 解析合并为结构化 JSON
   → 写入 data/source_cache/shared-football/parsed/{date}_{shuju_id}.json（单场独立）
...
抓取前先检查 `data/source_cache/shared-football/raw/{date}/{page}_{shuju_id}.html` 是否存在，存在则跳过网络请求。
...
> 集成方案: `python scripts/fetch_football_data.py --date DATE --shuju-id ID --deep`
...
```
原始HTML:  data/source_cache/shared-football/raw/{date}/{page}_{shuju_id}.html
  (每个页面独立缓存，其他技能可直接复用单页)

解析JSON:  data/source_cache/shared-football/parsed/{date}_{shuju_id}.json
  (每场比赛独立缓存，不互相污染)

抓取批次:  data/fetch_runs/{YYYYMMDD_HHMMSS}/
...
4. **竞彩官方赔率数据分层 (v2.8)**:
   - **主源**: sporttery.cn 页面抓取 (scripts/fetch_sporttery.py) → `data/source_cache/sporttery/{date}_jingcai.json`
...
```bash
# 单场
python scripts/fetch_football_data.py --date 2026-07-15 --shuju-id 1359162 --deep [--no-cache]

# 自动按球队筛选并抓取六页
python scripts/fetch_football_data.py --date 2026-07-15 --match 法国 --deep --no-cache
```

快速模式只抓中国竞彩和500比赛列表，不含ouzhi/yazhi/rangqiu/daxiao/shuju/touzhu深层数据。
...
|:---|:---|:---|
| `--date YYYY-MM-DD` | 竞彩业务日期；可对应次日凌晨开球 | 必填 |
| `--deep` | 抓取筛选比赛的六个深层页面 | 关闭 |
| `--shuju-id ID` | 显式指定单个或逗号分隔的ID | 自动发现 |
| `--no-cache` | 跳过缓存，强制刷新 | 使用缓存 |
| `--match NAME` | 按队伍名过滤 | 所有比赛 |
| `--sid ID` / `--round A` | 可选联赛赛季与轮次 | 不调用联赛API |
| `--output-root DIR` | 带时间戳的抓取批次根目录 | `data/fetch_runs/` |
...
在 Phase 0 Step 0a 中优先使用 Liansai API 获取比赛列表 + shuju_id:
  ...
  Step 4: 保存到 data/source_cache/shared-football/parsed/{date}_{shuju_id}.json
```

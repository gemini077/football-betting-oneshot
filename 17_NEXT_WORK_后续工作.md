...
Checkpoint 2026-08-25 refresh-chain audit: GitHub run 32746235810 for 2026-08-24 produced universe=13 and base_jobs=11 and completed the dashboard step, then build_public_site timed out at 360 seconds. Because the workflow save step used default success gating, those generated inputs were not committed; current durable latest remains 2026-08-23 and 2026-08-25 universe/jobs are absent. The bounded follow-up adds an explicit generated-data durability gate: Save generated public data runs after a site-only failure only when universe, jobs, prediction, dashboard and their current artifacts are complete; upstream failures remain fail-closed. Site timeout remains a separate blocker. No model, identity, registry, frozen history, ledger or result changes. ID3 remains CURRENT; this slice is READY_FOR_ACCEPTANCE only.
...
Checkpoint 2026-08-23: Competition/season identity bridge read-only blocker: the six latest fetch runs contained 5 target 500 deep artifacts with target team IDs 5/5, but explicit provider_competition_id=0/5 and provider_season_id=0/5; all 3,147 persisted fetch-run JSON artifacts had zero exact provider_competition_id/provider_season_id/competition_id/season_id/sid/lid fields. The raw 500 trade target row has structured fixture/team/process attributes but no target-level data-seasonid; data-seasonid appears only on separate league-filter spans, and URL-derived season IDs are excluded. Current persisted/parsed Nowscore schedule/market outputs contain match/team/kickoff data but no accepted competition/season IDs; raw bf1.js schedule columns were not persisted in this checkout, so their meanings remain unverified and this is not a claim about every possible upstream response. Deterministic provider-native joins across responses are legal only when an explicit stable key/foreign key links the same target match before cutoff with provenance and raw hash; text/date/name/URL/panlu inference remains illegal. Current 12 Champion input snapshots have provider competition ID=0/12 and provider season ID=0/12. The reviewed registry has 4 rows for openfootball/football-data.co.uk and 0 nowscore/500 rows. No code or identity coverage is added; ID3 remains CURRENT. Next priority is free/provider-native 500/Nowscore schema discovery before paid APIs; if proven, persist the target-linked provider competition+season evidence with match ID, response provenance, cutoff and hash, then resolve only through CompetitionEntityResolver.
...
Checkpoint 2026-08-23 follow-up: Nowscore acquisition now persists provider-native target SclassID and selectSeason evidence for future fetches; a read-only live proof found 12/12 target schedules, 6 future at proof time, 6/6 competition evidence, five SclassIDs (11/23/31/34/36), and 5/5 season keys (2026-2027). The default reviewed registry resolves 0/5 and leaves 5/5 unresolved; no production Nowscore registry rows were added. Existing frozen Champion rows remain untouched. This acquisition slice is READY_FOR_ACCEPTANCE at most; ID3 remains CURRENT pending reviewed registry evidence and independent acceptance.
Checkpoint 2026-08-23 audit/follow-up: audit found no P0; provider identity IDs without the complete verified evidence bundle now fail closed. Live post-freeze reads used only for audit confirmed SclassIDs 11/23/31/34/36 each return matching provider season key 2026-2027, but the default reviewed registry resolves 0/5; the 24 target provider team IDs have 0/24 existing reviewed crosswalk coverage. No registry rows or frozen rows were changed. The 23:59 read-only dry-run had 12 formal rows, 3 still future, 0 canonical competition/season resolutions, and 0 Challenger/pair artifacts; ID3 remains CURRENT.
# 1. 已完成，不再重跑

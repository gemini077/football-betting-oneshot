...
Usage:
  python scripts/fetch_and_parse.py --shuju-id 1359162 --date 2026-07-15
  python scripts/fetch_and_parse.py --shuju-id 1359162 --date 2026-07-15 --no-cache

Output: data/source_cache/shared-football/parsed/{date}_{shuju_id}.json
"""
...
DEFAULT_CACHE_DIR = os.path.join(PROJECT_ROOT, "data", "source_cache", "shared-football")
...
def parse_ouzhi(html_text):
    ...
    # Summary rows have stable element IDs. Parsing by IDs avoids layout text,
    # masked labels and nested-table ordering ambiguities. Suffix c=opening,
    # suffix j=current.
    def id_number(element_id):
        match = re.search(rf'id="{re.escape(element_id)}"[^>]*>\s*([\d.]+)', html_text)
        return float(match.group(1)) if match else None

    avg_open = {"home": id_number("avwinc2"), "draw": id_number("avdrawc2"), "away": id_number("avlostc2")}
    avg_current = {"home": id_number("avwinj2"), "draw": id_number("avdrawj2"), "away": id_number("avlostj2")}
    dispersion_open = {"home": id_number("lswc2"), "draw": id_number("lsdc2"), "away": id_number("lslc2")}
    dispersion_current = {"home": id_number("lswj2"), "draw": id_number("lsdj2"), "away": id_number("lslj2")}
    if all(value is not None for value in avg_open.values()):
        result["summary"]["avg_spf_open"] = avg_open
    if all(value is not None for value in avg_current.values()):
        result["summary"]["avg_spf_current"] = avg_current
    if all(value is not None for value in dispersion_open.values()):
        result["summary"]["dispersion_open"] = dispersion_open
    if all(value is not None for value in dispersion_current.values()):
        result["summary"]["dispersion_current"] = dispersion_current
        result["summary"]["dispersion"] = dispersion_current
...
def _company_market_rows(html_text):
    """Return complete outer bookmaker rows without being cut by nested tables."""
    pattern = re.compile(r'<tr\s+class="tr[12]"\s+xls="row"[^>]*>', re.IGNORECASE)
    starts = list(pattern.finditer(html_text))
    rows = []
    for index, match in enumerate(starts):
        end = starts[index + 1].start() if index + 1 < len(starts) else len(html_text)
        rows.append(html_text[match.start():end])
    return rows


def _line_market_table(table_html):
    """Parse the two prices and the numeric line carried in a market table."""
    cells = re.findall(r'<td[^>]*>(.*?)</td>', table_html, re.DOTALL)

    def leading_number(cell_html):
        text = re.sub(r'<[^>]+>', '', cell_html).strip()
        match = re.match(r'([\d.]+)', text)
        return float(match.group(1)) if match else None

    line_ref = re.search(r'\bref="(-?[\d.]+)"', table_html)
    line_cell = re.search(r'<td[^>]*\bref="-?[\d.]+"[^>]*>(.*?)</td>', table_html, re.DOTALL)
    line_text = None
    if line_cell:
        line_text = re.sub(r'<[^>]+>', '', line_cell.group(1)).strip()
    return {
        "first_price": leading_number(cells[0]) if len(cells) >= 1 else None,
        "second_price": leading_number(cells[2]) if len(cells) >= 3 else None,
        "line": float(line_ref.group(1)) if line_ref else None,
        "line_str": line_text or None,
    }
...
def parse_yazhi(html_text):
    ...
    # Outer rows contain nested <tr> elements. A non-greedy <tr> regex truncates
    # them at the first inner table row and silently drops lines/timestamps/opening data.
    rows = _company_market_rows(html_text)
    companies = []
    ...
    for row in rows:
        ...
        if not cid:
            row_id = re.search(r'<tr[^>]*\bid="(\d+)"', row)
            cid = int(row_id.group(1)) if row_id else 0

        co = OrderedDict([("name", name), ("cid", cid)])

        row_time = re.search(r'\bdt="([^"]+)"', row)
        if row_time:
            co["snapshot_time"] = row_time.group(1)

        tables = re.findall(r'<table[^>]*class="pl_table_data"[^>]*>.*?</table>', row, re.DOTALL)
        if tables:
            current = _line_market_table(tables[0])
            co["current_handicap"] = current["line"]
            co["current_handicap_str"] = current["line_str"]
            co["current_water_home"] = current["first_price"]
            co["current_water_away"] = current["second_price"]
        if len(tables) >= 2:
            opening = _line_market_table(tables[1])
            co["open_handicap"] = opening["line"]
            co["open_handicap_str"] = opening["line_str"]
            co["open_water_home"] = opening["first_price"]
            co["open_water_away"] = opening["second_price"]

        times = re.findall(r'<time>([^<]+)</time>', row)
        if times:
            co["change_time"] = times[0].strip()
        if len(times) >= 2:
            co["open_time"] = times[1].strip()
...
def parse_daxiao(html_text):
    ...
    for row in rows:
        ...
        if tables:
            current = _line_market_table(tables[0])
            co["current_line"] = abs(current["line"]) if current["line"] is not None else None
            co["current_line_str"] = current["line_str"]
            co["current_over_water"] = current["first_price"]
            co["current_under_water"] = current["second_price"]
        if len(tables) >= 2:
            opening = _line_market_table(tables[1])
            co["open_line"] = abs(opening["line"]) if opening["line"] is not None else None
            co["open_line_str"] = opening["line_str"]
            co["open_over_water"] = opening["first_price"]
            co["open_under_water"] = opening["second_price"]
...
def parse_touzhu(html_text):
    ...
    # Parse cells structurally. The old monolithic regex dropped the complete
    # panel whenever any optional index cell was "-".
    hot_start = html_text.find("热度分析")
    hot_end = html_text.find("数据提点", hot_start) if hot_start >= 0 else -1
    if hot_start >= 0 and hot_end > hot_start:
        section = html_text[hot_start:hot_end]
        ...
        for row_html in re.findall(r'<tr[^>]*>(.*?)</tr>', section, re.DOTALL):
            ...
            if len(cells) >= 11 and re.fullmatch(r'[\d.]+', cells[1]) and cells[2].endswith('%'):
        ...
        if len(data_rows) >= 3:
            ...
            for outcome, cells in zip(("home", "draw", "away"), data_rows[:3]):
                ...
                betfair[outcome] = OrderedDict([
                    ...
                    ("bookmaker_pl", page_simulated_pl),
                    ("betfair_index", optional_int(cells[8])),
                    ("hot_cold_index", optional_int(cells[9])),

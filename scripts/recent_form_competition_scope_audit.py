#!/usr/bin/env python3
"""Audit frozen recent-form competition scope and run one bounded challenger.

This research-only audit answers the pre-registered
RECENT-FORM-COMPETITION-SCOPE-OFFLINE-1 question.  It selects the exact
61-match cohort used by PR #163, proves the frozen recent-form aggregates from
the corresponding football evidence, joins competition labels only from the
same immutable Nowscore input snapshot, and only then permits the single
FRIENDLY_EXCLUDED counterfactual.
"""
...
MILESTONE = "RECENT-FORM-COMPETITION-SCOPE-OFFLINE-1"
...
# PR #163's recoverable artifact.  Keeping the pair, rather than only the
# match key, makes a changed snapshot selection fail closed.
EXPECTED_COHORT = {
...
def _load_frozen_prediction(
    ...
    if not isinstance(panlu_matches, list):
        raise ValueError(f"PANLU_MATCHES_MISSING:{prediction_id}")
    return {
        "path": _display_path(path),
        "prediction_id": prediction_id,
        "match_key": match_key,
...
def _join_competition(
    ...
    candidates = list(panlu_index.get(key, [])) if key is not None else []
    if len(candidates) != 1:
        return {
...
def _build_observation(
    ...
    reconstruction = _reconstruct_recent_form(
        windows,
        ((frozen["nowscore_snapshot"].get("shuju") or {}).get("recent_form")),
    )
    ...
    for component in COMPONENTS:
        ...
        for index, row in enumerate(windows[component]):
            join = _join_competition(row, panlu_index)
            internal = dict(row)
            internal["_join"] = join
...
def _build_competition_summary(observations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    joined_rows = unresolved_rows = ambiguous_rows = 0
    component_rows = 0
    friendly_rows = 0
...
def _decision(
    ...
    *,
    ...
    if evaluated_n < minimum_evaluable_unique_matches:
        return "COMPETITION_SCOPE_SAMPLE_INSUFFICIENT", {
            "evaluated_unique_matches": evaluated_n,
            "minimum_evaluable_unique_matches": minimum_evaluable_unique_matches,
            "structural_gate_passed": True,
    ...
    *,
    ...
    for evidence in selected_evidence:
        try:
            ...
            observations.append(_build_observation(evidence, frozen))
        except (KeyError, TypeError, ValueError) as exc:
            ...
            ineligible.append(
                {
                    ...
                    "reasons": sorted(
                        set(
                            ...
                            + (
                                ["COMPETITION_LABEL_UNRESOLVED"]
                                if not row["competition_join"]["all_resolved"]
                                else []
                            )
                        )
                    ),
    ...
    if decision == "COMPETITION_SCOPE_SAMPLE_INSUFFICIENT":
        status = "COMPETITION_SCOPE_SAMPLE_INSUFFICIENT"
    ...
    source = {
        "accepted_cohort_reference": {
            "pull_request": 163,
            "accepted_head": "793b2f09aff6cfec31591a61b37ac1d90b07167a",
            "artifact": 9893940729,
        ...
        "competition_truth": "same immutable input snapshot panlu.matches only",
    ...
    return _public_value(
        {
            "schema_version": "recent_form_competition_scope_audit.v1",
            "milestone": MILESTONE,
            "status": status,
            "decision": decision,
            "source": source,
            ...
            "pre_registered_decision_rules": {
                ...
                "COMPETITION_SCOPE_SAMPLE_INSUFFICIENT": f"outcome-blind eligible unique matches < {minimum_evaluable_unique_matches}",
...
def build_report(summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"- PR #163 accepted head: {((source.get('accepted_cohort_reference') or {}).get('accepted_head'))}",
        ...
        f"- identity matches PR #163: {cohort.get('identity_match_to_pr163')}",
        ...
        "",
        "Each component is reconstructed from the subject's deterministic latest-10 window in football_evidence and compared field-by-field on matches, goals_for, and goals_against with the same immutable input snapshot shuju.recent_form.",

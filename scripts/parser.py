...
用法: python scripts/parser.py [--date YYYY-MM-DD] [--xml-url URL]
...
DEFAULT_CACHE_DIR = os.path.join(PROJECT_ROOT, "data", "source_cache", "500-xml")
    ...
    for code, info in team_map.items():
        ...
        if len(parts) >= 1:
            ...
            try:
                ...
                year = args.date[:4] if args.date else str(datetime.now().year)

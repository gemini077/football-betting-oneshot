...
def build_review(schedule: dict, report: dict, now: datetime) -> dict:
    ...
    decisions = report.get("decisions") or {}
    primary_pick = _primary_outcome(decisions.get("unique_primary_dimension"))
    ...
    primary_hit = primary_pick == actual_outcome if primary_pick else None
    ...
    classification = "主维度命中" if primary_hit is True else "主维度错误" if primary_hit is False else "主维度不可结算"
    ...
    return {
        ...
        "settlement": {
            "primary": {"pick": primary_pick, "actual": actual_outcome, "hit": primary_hit, "actual_probability": probability},
        ...
        "复盘摘要": summary,
        "_timeline": {"盘口变化与赛果方向": _movement(report), "最终赛果验证": actual_score, "数据完整度": (report.get("data_quality") or {}).get("status")},

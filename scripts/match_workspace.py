...
function saveSelection(m){if(!isSelected(m))selected.push({id:m.id,match_num:m.match_num,home:m.home,away:m.away,league:m.league,kickoff:m.kickoff,selected_at:new Date().toISOString(),analysis_requested:false});localStorage.setItem('fbos-analysis-selections',JSON.stringify(selected));fetch('http://127.0.0.1:8765/v1/analysis-selections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match:m})}).catch(()=>{});render()}
function upcomingRow(m){const spf=m.spf||{};return `<tr data-row="${key(m)}"><td><b>${m.kickoff||'—'}</b><div class="muted">${m.match_num||'—'} · ${m.league||'—'}</div></td><td><div class="match-name">${m.home} <span class="muted">vs</span> ${m.away}</div><div class="muted">${m.official?'体彩在售':'额外关注'}</div></td><td><div class="odds-inline"><span class="odd">胜 ${spf.home??'—'}</span><span class="odd">平 ${spf.draw??'—'}</span><span class="odd">负 ${spf.away??'—'}</span></div></td><td><span class="badge ${m.report_state==='已分析'?'good':'blue'}">${m.report_state}</span> ${isSelected(m)?'<span class="badge gold">已选择</span>':''}</td><td><b>${m.primary||'—'}</b><div class="risk-line">错点：${m.primary_error||'—'}</div><div class="muted">${m.betting_state||'未锁单'}</div></td><td><div class="actions"><button class="action ${isSelected(m)?'selected':'primary'}" data-select="${key(m)}">${isSelected(m)?'✓ 待分析':'加入待分析'}</button><button class="action" data-open="${key(m)}">${m.report_url?'打开报告':'查看状态'}</button></div></td></tr>`}

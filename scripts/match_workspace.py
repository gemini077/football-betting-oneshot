...
def prediction_layer(report: dict | None) -> dict:
    ...
    missing = [item for item in quality.get("missing") or [] if item]
    decisions = payload.get("decisions") or {}
    ...
    if not missing and count >= 4:
        grade, weight = "A", 1.0
    elif len(missing) <= 1 and count >= 2:
        grade, weight = "B", 0.7
    else:
        grade, weight = "C", 0.4
    formal = grade in {"A", "B"} and bool(decisions.get("unique_primary_dimension"))
    betting = payload.get("betting") or {}
    return {"research": True, "data_grade": grade, "calibration_weight": weight,
...
(function(){
  ...
  const cs=DATA.calibration_status||{},cm=cs.metrics||{},cw=cs.weighted_primary||{};
  calibration.innerHTML=`<div class="card-title"><div><div class="section-kicker">CALIBRATION LOOP</div><h2>全量预测与校准状态</h2></div><span>不减少研究样本</span></div><div class="paper-kpis"><div class="paper-kpi"><b>${(cs.scope||{}).deduplicated_matches||0}</b><span>去重比赛</span></div><div class="paper-kpi"><b>${(cs.quality_distribution||{}).A||0}/${(cs.quality_distribution||{}).B||0}</b><span>A/B正式层</span></div><div class="paper-kpi"><b>${rate(cm.primary?.hit_rate)}</b><span>主维度命中</span></div><div class="paper-kpi"><b>${rate(cm.exact_score?.hit_rate)}</b><span>唯一比分命中</span></div><div class="paper-kpi"><b>${Object.keys(cs.triggers||{}).length}</b><span>错误标签</span></div><div class="paper-kpi"><b>${rate(cw.hit_rate)}</b><span>加权主维度</span></div></div><p class="paper-note">所有比赛保留为研究样本；A/B级才可进入正式首推，C级降权校准，D级隔离。错误标签达到5/10/20场时分别进入观察、候选修正和升级评估。</p>`;

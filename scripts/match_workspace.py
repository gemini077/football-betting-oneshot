...
const requestingAnalysis=new Set();
async function saveSelection(m){if(!m)return;if(m.report_state==='已分析'){if(m.report_url)openMatch(m,'prematch');return}const id=key(m);if(requestingAnalysis.has(id))return;requestingAnalysis.add(id);render();try{const response=await fetch('http://127.0.0.1:8765/v1/analysis-selections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({match:{...m,business_date:m.business_date||DATA.target_date}})});const result=await response.json().catch(()=>({}));if(!response.ok||!result.ok)throw new Error(result.error||`HTTP ${response.status}`);selected=selected.filter(x=>String(x.id)!==id);selected.push({id:m.id,match_num:m.match_num,home:m.home,away:m.away,league:m.league,kickoff:m.kickoff,selected_at:new Date().toISOString(),analysis_requested:true,request_status:'queued'});localStorage.setItem('fbos-analysis-selections',JSON.stringify(selected));}catch(error){alert(`未能提交分析：${error.message||error}\n请确认本地桥接已启动，页面不会跳转 GitHub。`)}finally{requestingAnalysis.delete(id);render()}}
function upcomingRow(m){const spf=m.spf||{},analyzed=m.report_state==='已分析',hasReport=Boolean(m.report_url);const actions=hasReport?`<button class="action primary" data-open="${key(m)}">打开报告</button>${analyzed?`<button class="action" data-bet="${key(m)}">已下单</button>`:`<button class="action" data-select="${key(m)}">重新分析</button>`}`:`<button class="action ${isSelected(m)?'selected':'primary'}" data-select="${key(m)}">${isSelected(m)?'重新提交分析':'分析这场'}</button>`;return `<tr data-row="${key(m)}"><td><b>${m.kickoff||'—'}</b><div class="muted">${m.match_num||'—'} · ${m.league||'—'}</div></td><td><div class="match-name">${m.home} <span class="muted">vs</span> ${m.away}</div><div class="muted">${m.official?'体彩在售':'额外关注'}</div></td><td><div class="odds-inline"><span class="odd">胜 ${spf.home??'—'}</span><span class="odd">平 ${spf.draw??'—'}</span><span class="odd">负 ${spf.away??'—'}</span></div></td><td><span class="badge ${analyzed?'good':'blue'}">${m.report_state}</span> ${!analyzed&&isSelected(m)?'<span class="badge gold">已提交</span>':''}</td><td><b>${m.primary||'—'}</b><div class="risk-line">错点：${m.primary_error||'—'}</div><div class="muted">${m.betting_state||'未锁单'}</div></td><td><div class="actions">${actions}</div></td></tr>`}
...
(function(){
  ...
  upcomingRow=m=>{const spf=m.spf||{},analyzed=m.report_state==='已分析',hasReport=Boolean(m.report_url),requesting=requestingAnalysis.has(key(m)),queued=isSelected(m);const analysisAction=requesting?'<button class="action selected" disabled>\u6b63\u5728\u63d0\u4ea4…</button>':queued?'<button class="action selected" disabled>\u5df2\u52a0\u5165\u5f85\u5206\u6790</button>':`<button class="action primary" data-select="${key(m)}">${hasReport?'\u91cd\u65b0\u5206\u6790':'\u52a0\u5165\u5f85\u5206\u6790'}</button>`;const betAction=m.bet_locked?'<button class="action selected" disabled>\u5df2\u786e\u8ba4\u771f\u5b9e\u4e0b\u6ce8</button>':`<button class="action" data-bet="${key(m)}">\u767b\u8bb0\u5b9e\u9645\u4e0b\u6ce8</button>`;const actions=hasReport?`<button class="action primary" data-open="${key(m)}">\u6253\u5f00\u62a5\u544a</button>${analyzed?betAction:analysisAction}`:analysisAction;return `<tr data-row="${key(m)}"><td><b>${m.kickoff||'\u2014'}</b><div class="muted">${m.match_num||'\u2014'} \u00b7 ${m.league||'\u2014'}</div></td><td><div class="match-name">${m.home} <span class="muted">vs</span> ${m.away}</div><div class="muted">${m.official?'\u4f53\u5f69\u5728\u552e':'\u989d\u5916\u5173\u6ce8'}</div></td><td><div class="odds-inline"><span class="odd">\u80dc ${spf.home??'\u2014'}</span><span class="odd">\u5e73 ${spf.draw??'\u2014'}</span><span class="odd">\u8d1f ${spf.away??'\u2014'}</span></div></td><td><span class="badge ${analyzed?'good':'blue'}">${m.report_state}</span> ${queued?'<span class="badge gold">\u5df2\u6392\u961f</span>':''}</td><td><b>${m.primary||'\u2014'}</b><div class="risk-line">\u9519\u70b9\uff1a${m.primary_error||'\u2014'}</div><div class="muted">${m.bet_locked?'\u5df2\u786e\u8ba4\u771f\u5b9e\u4e0b\u6ce8':(m.betting_state||'\u672a\u4e0b\u5355')}</div></td><td><div class="actions">${actions}</div></td></tr>`};

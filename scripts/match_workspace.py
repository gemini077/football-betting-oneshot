...
const DATA=''' + embedded + ''';let mode='all',current=null,selected=JSON.parse(localStorage.getItem('fbos-analysis-selections')||'[]');const $=s=>document.querySelector(s),key=m=>String(m.id),selectionFor=m=>selected.find(x=>String(x.id)===key(m)),isSelected=m=>Boolean(selectionFor(m));$('#subtitle').textContent=`${DATA.model_version} · 体彩已开售 ${DATA.target_date} 起两个业务日 · 更新 ${new Date(DATA.generated_at).toLocaleString()}`;$('#scheduleMeta').textContent=`${DATA.target_date} 起 · ${DATA.matches.length} 场`;$('#balance').textContent=`¥${Number(DATA.balance||0).toFixed(2)}`;$('#openBetCount').textContent=DATA.open_bets.length;$('#completedCount').textContent=DATA.completed.length;
const ANALYSIS_ENDPOINT='http://127.0.0.1:8765/v1/analysis-selections';let queueFlushRunning=false;const persistSelections=()=>localStorage.setItem('fbos-analysis-selections',JSON.stringify(selected));

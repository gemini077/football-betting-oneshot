...
(function(){
  ...
  const ticketRows=rows=>rows.map(t=>{const sm=statusMeta(t);return `<tr><td><b>${t.match}</b><div class="muted">${t.model_version||'—'} · ${t.ticket_id}</div></td><td>${t.market}<div class="muted">${t.price_source||'无有效赛前价格'}</div></td><td>${t.odds==null?'—':Number(t.odds).toFixed(2)}</td><td>${Number(t.stake_units||0).toFixed(2)}</td><td class="${sm[1]}">${sm[0]}</td><td class="${tone(t.profit_units)}">${t.profit_units==null?'—':signed(t.profit_units)+' 元'}</td></tr>`;}).join('')||'<tr><td colspan="6" class="empty-row">生成赛前报告后，模拟注单会自动出现在这里</td></tr>';
  ...
  paperDialog.innerHTML=`<div class="dialog-head"><div><b>全部模拟注单</b><div class="muted">共 ${allPaperTickets.length} 条 · 方向、赔率与金额按赛前冻结记录展示</div></div><button type="button" class="close" id="closePaperTickets">关闭</button></div><div class="table-wrap"><table class="compact-table"><thead><tr><th>比赛</th><th>冻结合约</th><th>赔率</th><th>金额</th><th>结算</th><th>盈亏</th></tr></thead><tbody>${ticketRows(allPaperTickets)}</tbody></table></div><p class="paper-dialog-note">模拟投注最低2.00元，步进0.01元；观察无价记录不计入盈亏。</p>`;
